...
export default function DashboardMockup() {
  return (
    <div className="w-full max-w-[1000px] mx-auto rounded-3xl overflow-hidden shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] border border-gray-200 ring-1 ring-black/5 bg-[#fafafa]">
      {/* Browser chrome */}
      ...
      <div className="p-8">
        <div className="flex justify-between items-center mb-8">
          <div>
             <h1 className="text-2xl font-black text-gray-900">Intelligence Center</h1>
        ...
        <div className="grid grid-cols-4 gap-5 mb-8">
          {[
            ...
            { label: "Active Live Chats", val: "3", badge: "Live now", color: "text-orange-600", bg: "bg-orange-50" },
            ...
            <div key={s.label} className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-4">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${s.bg}`}>
                   {s.label.includes("Interaction") ? <PhoneCall size={16} className={s.color} /> : <MessageSquare size={16} className={s.color} />}
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm h-72 flex flex-col relative overflow-hidden">
             ...
             <div className="flex-1 w-full relative flex items-end justify-between px-2 pt-10 z-10">
               ...
               {[{tot: 60, ai: 40, label: "Mon"}, {tot: 80, ai: 50, label: "Tue"}, {tot: 40, ai: 20, label: "Wed"}, {tot: 90, ai: 70, label: "Thu"}, {tot: 70, ai: 50, label: "Fri"}, {tot: 30, ai: 30, label: "Sat"}, {tot: 20, ai: 20, label: "Sun"}].map((h, i) => (
                  <div key={i} className="relative w-[8%] h-full flex items-end justify-center group cursor-pointer z-10">
                    <div className="absolute -bottom-8 text-xs font-semibold text-gray-400">{h.label}</div>
                    
                    {/* Light Gray (Total) */}
                    <div className="absolute bottom-0 w-full bg-gray-200 rounded-t-md transition-all group-hover:bg-gray-300" style={{ height: `${h.tot}%` }} />
                    {/* Blue (AI) */}
                    <div className="absolute bottom-0 w-full bg-blue-600 rounded-t-md transition-all group-hover:bg-blue-500 shadow-[0_0_15px_rgba(37,99,235,0.2)]" style={{ height: `${h.ai}%` }} />
